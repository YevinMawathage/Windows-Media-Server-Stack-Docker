# Ultimate Windows Media Server Stack (Docker)
...
* **Set Categories:** In the left sidebar, right-click and create two categories: `tv-sonarr` (Save path: `/downloads`) and `movies-radarr` (Save path: `/downloads`).
...
In both Sonarr and Radarr, go to Settings -> Download Clients -> Add (+) -> qBittorrent. Set the Host to `qbittorrent` and the Port to `8082`. Set the Category to `tv-sonarr` (for Sonarr) and `movies-radarr` (for Radarr).
...
### Link to Plex
In Sonarr/Radarr, go to Settings -> Connect -> Add (+) -> Plex Media Server. Set Host to `plex` and Port to `32400`. Authenticate your account and check "On Download" and "On Upgrade".
